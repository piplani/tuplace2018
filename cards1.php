...
if(isset($_GET)) {
	...
	echo '<div class="container">Showing '.$result->num_rows.' matching project(s)<hr><div class="row">';
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$team = $row['size'];
			$title = (strlen($row['title']) > 100) ? ucwords(substr($row['title'],0,100))."..." : ucwords($row['title']);
			$summary = (strlen($row['summary']) > 450) ? substr($row['summary'],0,450)."..." : $row['summary'];

			echo
			'<div class="col-lg-12 col-sm-12 portfolio-item my-3 card-outer">
				<div class="card list-card h-100 px-2" style="cursor: default;">
					<div class="card-body">
						<h4 class="card-title" style="color:#F44336;">P1'.$row['pid'].': '.$title.'</h4>';
			echo
						'	<p class="card-text card-text-min">'.$summary.'</p><hr>';

			echo '<a class="ui button" href = project.php?id=' . $row['pid'] . '>Browse</a>';
			if($row['link']) echo '<a class="ui button" href = ' . $row['link'] . ' target="_new">View Project</a>';
